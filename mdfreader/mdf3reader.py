...
from .mdf import MdfSkeleton, _open_mdf, dataField, conversionField, idField, CompressedData
...
def _tab_interp_conversion(data, conversion):  # 1 Tabular with interpolation
    ...
    converted data to physical value
    """
    tmp = array([(key, val["int"], val["phys"]) for (key, val) in conversion.items()])
...
def _exponential_conversion(data, conversion):  # 7 Exponential
    ...
    if conversion["P4"] == 0 and conversion["P1"] != 0 and conversion["P2"] != 0:
        return (
            exp(((data - conversion["P7"]) * conversion["P6"] - conversion["P3"]) / conversion["P1"]) / conversion["P2"]
        )
    elif conversion["P1"] == 0 and conversion["P4"] != 0 and conversion["P5"] != 0:
        return (
            exp((conversion["P3"] / (data - conversion["P7"]) - conversion["P6"]) / conversion["P4"]) / conversion["P5"]
        )
    else:
...
def _log_conversion(data, conversion):  # 8 Logarithmic
    ...
    if conversion["P4"] == 0 and conversion["P1"] != 0 and conversion["P2"] != 0:
        return (
            log(((data - conversion["P7"]) * conversion["P6"] - conversion["P3"]) / conversion["P1"]) / conversion["P2"]
        )
    elif conversion["P1"] == 0 and conversion["P4"] != 0 and conversion["P5"] != 0:
        return (
            log((conversion["P3"] / (data - conversion["P7"]) - conversion["P6"]) / conversion["P4"]) / conversion["P5"]
        )
    else:
...
def _formula_conversion(data, conversion):  # 10 Text Formula
    ...
    try:
        ...
        warn("Failed to convert formulae " + conversion["textFormula"] + " Sympy is correctly installed ?\n")
...
class Record(list):
        ...
        for channelNumber in range(info["CGBlock"][self.dataGroup][self.channelGroup]["numberOfChannels"]):
            ...
            if len(self) > 1:
                ...
                prev_chan_includes_curr_chan = (
                    channel.posBitBeg >= 8 * prev_chan.byteOffset
                    and channel.posBitEnd <= 8 * (prev_chan.byteOffset + prev_chan.nBytes_aligned)
                )
                if embedding_channel is not None:
                    embedding_channel_includes_curr_chan = channel.posBitEnd <= embedding_channel.posByteEnd * 8
                else:
                    embedding_channel_includes_curr_chan = False
                if (
                    channel.byteOffset >= prev_chan.byteOffset
                    and channel.posBitBeg < 8 * (prev_chan.byteOffset + prev_chan.nBytes_aligned)
                    and channel.posBitEnd > 8 * (prev_chan.byteOffset + prev_chan.nBytes_aligned)
                ):
                    # not byte aligned
                    self.byte_aligned = False
                if embedding_channel is not None and channel.posBitEnd > embedding_channel.posByteEnd * 8:
                    embedding_channel = None
                if prev_chan_includes_curr_chan or embedding_channel_includes_curr_chan:  # bit(s) in byte(s)
                    ...
                    if self.recordToChannelMatching:  # not first channel
                        self.recordToChannelMatching[channel.name] = self.recordToChannelMatching[prev_chan.name]
    ...
    def read_sorted_record(self, fid, pointer, channel_set=None):
        ...
        if channel_set is None and not self.hiddenBytes and self.byte_aligned:
            ...
            if len(channel_set & self.channelNames) > 0:
                ...
                rec = recarray(
                    (self.numberOfRecords,), dtype={"names": data_record_name, "formats": numpy_data_record_format}
                )
                    ...
                    try:  # use rather cython compiled code for performance
                        ...
                        for n_record_chunk, chunk_size in chunks:
                            ...
                            for id, chan in enumerate(rec_chan):
                                rec[chan.name][previous_index : previous_index + n_record_chunk] = sorted_data_read(
                                    bytes(bit_stream),
                                    chan.bitCount,
                ...
                for r in range(self.numberOfRecords):  # for each record,
                    ...
                    for channel in rec_chan:
                        (rec[channel.name][r],) = channel.CFormat.unpack(buf[channel.posByteBeg : channel.posByteEnd])
    ...
    def read_record_buf(self, buf, channel_set=None):
        ...
        for Channel in self:  # list of channel classes from channelSet
            if Channel.name in channel_set:
                temp[self.recordToChannelMatching[Channel.name]] = Channel.CFormat.unpack(
                    buf[Channel.posByteBeg : Channel.posByteEnd]
                )[0]
    ...
    def read_record_bits(self, bit_stream, channel_set=None):
        ...
        for Channel in self:  # list of channel classes from channelSet
            if Channel.name in channel_set:
                ...
                if (
                    Channel.signalDataType in (1, 10, 14) and n_bytes == Channel.nBytes_aligned and n_trail_bits > 0
                ):  # Ctype byte length but signed integer
...
class DATA(dict):
    ...
    def read(self, channel_set, file_name):
        ...
        if len(self) == 1:  # sorted dataGroup
            ...
            self[record_id]["data"] = self.load_sorted(self[record_id]["record"], name_list=channel_set)
            ...
            for record_id in self:
                ...
                for channel in self[record_id]["record"]:
                    if self[record_id]["record"].recordToChannelMatching[channel.name] in data:
                        self[record_id]["data"][channel.name] = data[
                            self[record_id]["record"].recordToChannelMatching[channel.name]
                        ]
            ...
            for Channel in self[record_id]["record"]:
                if Channel.name in channel_name_set[record_id]:
                    buf[Channel.name] = empty(
                        (self[record_id]["record"].numberOfRecords,), dtype="V{}".format(Channel.nBytes_aligned)
                    )
...
class Mdf3(MdfSkeleton):
    ...
    def read3(
        ...
        if info is None:
            if self.info is None:
                info = Info3(self.fileName, fid=None, filter_channel_names=filter_channel_names, minimal=minimal)
            else:
                info = self.info
        ...
        for dataGroup in data_groups:
            ...
            if info["DGBlock"][dataGroup]["numberOfChannelGroups"] > 0 and (
                ...
                if channel_set is not None:  # making sure there are also masters
                    for recordID in buf:
                        if (
                            buf[recordID]["record"].channelNames & channel_set
                            and buf[recordID]["record"].master["name"] not in channel_set
                        ):
                ...
                if self._noDataLoading and channel_list is not None:
                    channel_groups = [
                        info["CGBlock"][dataGroup][self[channel][idField][1]]["recordID"] for channel in channel_list
                    ]
                ...
                for recordID in channel_groups:
                    if recordID in buf and "record" in buf[recordID]:
                        ...
                        if channel_list is None or not self._noDataLoading:
                            channels = (
                                c for c in buf[recordID]["record"] if channel_set is None or c.name in channel_set
                            )
                        else:
                        ...
                        for chan in channels:  # for each channel
                            ...
                            if (
                                ...
                                record_name = buf[recordID]["record"].recordToChannelMatching[chan.name]
                            else:
    ...
    def write3(self, file_name=None):
        ...
        head = (b"MDF     ", b"3.30    ", b"MDFreadr", 0, 0, 330, 28591, b"\0" * 32)
        ...
        head = (
            ...
            project,
            subject,
            int(self.fileMetadata["time"] * 1000000000),
            1,
            0,
        ...
        for masterChannel in self.masterChannelList:
            ...
            for channel in self.masterChannelList[masterChannel]:
                ...
                if preceeding_channel is not None:  # not possible for first CN
                    ...
                    write_pointer(
                        fid,
                        pointers["CN"][data_group][preceeding_channel]["nextCN"],
                ...
                try:
                    unit = "{:\x00<20.19}".format(self.get_channel_unit(channel)).encode("latin-1", "replace")
                except:
                    unit = b"\x00" * 20
                head = (b"CC", 46, value_range_valid, minimum, maximum, unit, 65535, 0)
