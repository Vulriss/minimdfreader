...
def time_val(func):
    """Time execution function, used as decorator."""

    def wrapper(*args, **kwargs):
        _begin_time = process_time()
        res = func(*args, **kwargs)
        print("{}: {:.5f}s".format(func.__name__, process_time() - _begin_time))
        return res
...
def _data_block(record, info, parent_block, channel_set=None, n_records=None, sorted_flag=True, vlsd=None):
    ...
    if parent_block["id"] in (b"##DT", b"##DV", b"##RD", "##DT", "##RD", "##DV"):  # normal data block
        if sorted_flag:
            if (
                channel_set is None and not record.hiddenBytes and record.byte_aligned
            ):  # No channel list and length of records corresponds to C datatypes
                ...
                if info["DG"][record.dataGroup]["unique_channel_in_DG"] and parent_block["id"] in (b"##DV", "##DV"):
                    return frombuffer(
                        parent_block["data"],
                        dtype={"names": record.dataRecordName, "formats": record.numpyDataRecordFormat},
                    )
                else:
                    return fromstring(
                        parent_block["data"],
        ...
        return _read_sd_block(
            record[record.VLSD[0]].signal_data_type(info),
            parent_block["data"],
        ...
        # uncompress data
        parent_block["data"] = DZBlock.decompress_data_block(
            parent_block["data"],
            parent_block["dz_zip_type"],
        ...
        return frombuffer(
            parent_block["data"],
            dtype={"names": [record.invalid_channel.name], "formats": [record.invalid_channel.data_format(info)]},
        )
...
def _read_unsorted(record, info, parent_block, record_id_size):
    ...
    for record_id in record:
        if record[record_id]["record"].Flags & 0b1:
            ...
            VLSD_CG_signal_data_type[record_id] = (
                record[record_id]["record"].VLSD_CG[record_id]["channel"].signal_data_type(info)
            )
        else:
            VLSD_flag[record_id] = False
            for Channel in record[record_id]["record"].values():
                # if not Channel.VLSD_CG_Flag:
                buf[Channel.name] = empty(
                    (record[record_id]["record"].numberOfRecords,), dtype="V{}".format(Channel.nBytes_aligned)
                )
    ...
    while position < data_block_length:
        ...
        if not VLSD_flag[record_id]:  # not VLSD CG
            for channel_name in channel_name_set[record_id]:  # list of channel classes from channelSet
                buf[channel_name][index[record_id]] = parent_block["data"][
                    position + pos_byte_beg[channel_name] : position + pos_byte_end[channel_name]
                ]
            index[record_id] += 1
...
def _read_sd_block(signal_data_type, sd_block, sd_block_length, n_records, pointer):
    ...
    if dataRead_available:
        try:
            return sd_data_read(signal_data_type, memoryview(sd_block).tobytes(), sd_block_length, n_records)
        except Exception as e:
            warn("data_read cython module - sd_data_read function crashed, using python based parsing backup")
    VLSDLen = diff(pointer) - 4
    VLSDLen = concatenate(
        (VLSDLen, array([sd_block_length - pointer[-1] - 4]).astype(dtype="<u{}".format(pointer.dtype.itemsize))),
        axis=0,
    )
    ...
    if max_len > 0:
        if signal_data_type < 10:
            ...
            for index, position in enumerate(pointer):
                ...
                output[index] = (
                    sd_block[position : int(position + VLSDLen[index])].decode(channel_format).rstrip("\x00")
                )
...
class Data(dict):
    ...
    def read(self, channel_set, info, filename):
        ...
        if len(self) == 1:  # sorted dataGroup
            ...
            self[recordID]["data"], self[recordID]["invalid_data"] = self.load(
                record, info, name_list=channel_set, sorted_flag=True
            )
            if record.VLSD:  # VLSD channels exist
                ...
                for cn in record.VLSD:  # VLSD channels
                    if channel_set is None or record[cn].name in channel_set:
                        ...
                        temp, invalid = temp.load(
                            record,
                            info,
                            name_list=channel_set,
                            sorted_flag=True,
                            vlsd=pointer.view(dtype="<u{}".format(pointer.dtype.itemsize)),
                        )
                        if temp is not None:
                            # change channel name by appending offset
                            self[recordID]["data"] = rename_fields(
                                self[recordID]["data"], {record[cn].name: "{}_offset".format(record[cn].name)}
                            )
    ...
    def load(self, record, info, name_list=None, sorted_flag=True, vlsd=None):
        ...
        if temps["id"] in (b"##DV", "##DV"):
            ...
            while temps["next"]:  # reads pointers to all data blocks (DT, RD, SD, DZ)
                ...
                temps["list_data"][index] = structunpack(
                    "<{}Q".format(temp["link_count"] - 1), self.fid.read(8 * (temp["link_count"] - 1))
                )
                index += 1
            if temps["count"]:
                ...
                if vlsd is not None or not sorted_flag:
                    ...
                    temps["data"] = _data_block(
                        record,
                        info,
                        parent_block=data_block,
                        channel_set=name_list,
                        n_records=None,
                        sorted_flag=sorted_flag,
                        vlsd=vlsd,
                    )
                else:
                    temps["data"] = self.read_data_list(
                        "list_data", record.CGrecordLength, temps, record, info, name_list, sorted_flag, vlsd
                    )
                    if temps["id"] in (b"##LD", "##LD"):
                        try:  # invalid bytes in DIBlock
                            temps["invalid_data"] = self.read_data_list(
                                "inval_data",
            ...
            temps["data"], temps["invalid_data"] = self.load(
                record, info, name_list=name_list, sorted_flag=sorted_flag, vlsd=vlsd
            )
            ...
            if sorted_flag:  # normal sorted data block, direct read
                ...
                temps["data"] = _data_block(
                    record, info, parent_block=temps, channel_set=name_list, n_records=None, sorted_flag=sorted_flag
                )
...
class Record(dict):
    ...
    def __str__(self):
        ...
        output.append(
            ...
            "\nByte aligned : {}\nHidden bytes : {}\n".format(
                self.CGrecordLength, self.dataGroup, self.byte_aligned, self.hiddenBytes
            )
        )
            ...
            if channel_type in (0, 2, 4, 5):  # not virtual channel
                ...
                if signal_data_type == 13:
                    ...
                    if len(self) > 1:
                        ...
                        if (
                            channel.byteOffset >= prev_chan_byte_offset
                            and channel.pos_bit_beg
                            < 8 * (prev_chan_byte_offset + prev_chan_n_bytes)
                            < channel_pos_bit_end
                        ):
                            # not byte aligned
                            self.byte_aligned = False
                        if (
                            embedding_channel is not None
                            and channel_pos_bit_end > embedding_channel.pos_byte_end(info) * 8
                        ):
                            embedding_channel = None
                        if prev_chan_includes_curr_chan or embedding_channel_includes_curr_chan:  # bit(s) in byte(s)
                            ...
                            if self.recordToChannelMatching:  # not first channel
                                self.recordToChannelMatching[channel.name] = self.recordToChannelMatching[
                                    prev_chan.name
                                ]
                            else:  # first channels
    ...
    def read_all_channels_sorted_record(self, fid):
        ...
        previous_index = 0
        buf = recarray(
            self.numberOfRecords, dtype={"names": self.dataRecordName, "formats": self.numpyDataRecordFormat}
        )  # initialise array
        simplefilter("ignore", FutureWarning)
        for n_record_chunk, chunk_size in chunks:
            buf[previous_index : previous_index + n_record_chunk] = fromstring(
                fid.read(chunk_size),
    ...
    def read_unique_channel(self, fid, info):
        ...
        """
        nbytes = (
            info["CG"][self.dataGroup][self.channelGroup]["cg_data_bytes"]
            * info["CG"][self.dataGroup][self.channelGroup]["cg_cycle_count"]
        )
        return frombuffer(fid.read(nbytes), dtype={"names": self.dataRecordName, "formats": self.numpyDataRecordFormat})
    ...
    def read_not_all_channels_sorted_record(self, fid, info, channel_set):
        ...
        if rec is not None:
            if dataRead_available:
                for n_record_chunk, chunk_size in chunks:
                    rec[previous_index : previous_index + n_record_chunk] = self.read_channels_from_bytes(
                        fid.read(chunk_size), info, channel_set, n_record_chunk, rec.dtype, channels_indexes
                    )
                    previous_index += n_record_chunk
                return rec
            else:
                for n_record_chunk, chunk_size in chunks:
                    rec[previous_index : previous_index + n_record_chunk] = self.read_channels_from_bytes_fallback(
                        fid.read(chunk_size), info, channel_set, n_record_chunk, rec.dtype, channels_indexes
                    )
                    previous_index += n_record_chunk
                return rec
    ...
    def read_record_buf(self, buf, info, channel_set=None):
        ...
        for Channel in self.values():  # list of channel classes from channelSet
            if Channel.name in channel_set and not Channel.VLSD_CG_Flag:
                temp[Channel.name] = Channel.c_format_structure(info).unpack(
                    buf[Channel.pos_byte_beg(info) : Channel.pos_byte_end(info)]
                )[0]
    ...
    def read_channels_from_bytes(
        self, bit_stream, info, channel_set=None, n_records=None, dtype=None, channels_indexes=None
    ):
            ...
            if dataRead_available:  # use rather cython compiled code for performance
                ...
                for chan in channels_indexes:
                    if self[chan].is_ca_block(info):
                        ...
                        array_flag = 0
                    buf[self[chan].name] = sorted_data_read(
                        bytes_data,
                        self[chan].bit_count(info),
    ...
    def read_channels_from_bytes_fallback(
        self, bit_stream, info, channel_set=None, n_records=None, dtype=None, channels_indexes=None
    ):
        ...
        if buf is not None:
            ...
            for chan in channels_indexes:
                ...
                if not self[chan].type in (1, 2):
                    temp = [
                        bit_array[
                            self[chan].pos_bit_beg
                            + record_bit_size * i : self[chan].pos_bit_end(info)
                            + record_bit_size * i
                        ]
                        for i in range(n_records)
                    ]
                    n_bytes = len(temp[0].tobytes())
                    if not n_bytes == n_bytes_estimated and signal_data_type not in (
                        6,
                        7,
                        8,
                        9,
                    ...
                    if (
                        signal_data_type in (2, 3) and n_bytes == n_bytes_estimated and n_trail_bits > 0
                    ):  # C type byte length but signed integer
                    ...
                    temp = [
                        bit_array[
                            self[chan].pos_bit_beg
                            + record_bit_size * i : self[chan].pos_bit_beg
                            + 8 * n_bytes_estimated
                            + record_bit_size * i
                        ]
                        for i in range(n_records)
                ...
                if "s" not in self[chan].c_format(info):
                    ...
                    temp = [temp[i].tobytes() for i in range(n_records)]
                    temp = asarray(temp)
...
class Mdf4(MdfSkeleton):
    ...
    def read4(
        ...
        if info is None:
            if self.info is None:
                info = Info4(self.fileName, None, filter_channel_names=filter_channel_names, minimal=minimal)
            else:
                info = self.info
        ...
        for dataGroup in data_groups:
            ...
            if not info["DG"][dataGroup]["dg_data"] == 0 and (
                channel_set is None or len(channel_set & info["ChannelNamesByDG"][dataGroup]) > 0
            ):  # there is data block and channel in
                ...
                if data_existing_in_data_group:
                    ...
                    if "dataClass" not in info["DG"][dataGroup]:
                        ...
                        for channelGroup in info["CG"][dataGroup]:
                            ...
                            if temp.master is not None and buf[record_id]["record"].channelNames:
                                if (
                                    channel_set is not None
                    ...
                    if self._noDataLoading and channel_list is not None:
                        channel_groups = [
                            info["CG"][dataGroup][self[channel][idField][0][1]]["cg_record_id"]
                                ...
                                if channel_set is None or chan.name in channel_set:
                                    if not chan.type == 4:  # normal channel
                                        if chan.channel_type(info) not in (3, 6):  # not virtual channel
                                            ...
                                            if (
                                                channel_set is None
                                                and not buf[record_id]["record"].hiddenBytes
                                        ...
                                        if (
                                            buf[record_id]["record"].byte_aligned
                                            and not buf[record_id]["record"].hiddenBytes
                                            and channel_set is None
                                            and 0 < bit_count < 64
                                            and bit_count not in (8, 16, 32)
                                            ...
                                            if signal_data_type in (0, 1, 2, 3):  # integers
                                                ...
                                                warn(
                                                    "bit count and offset not applied to correct "
                                        ...
                                        if temp is not None:  # channel contains data
                                            ...
                                            self.add_channel(
                                                ...
                                                info=chan.cn_block(info),
                                                compression=compression,
                                                identifier=info.unique_id(
                                                    chan.dataGroup, chan.channelGroup, chan.channelNumber
                                                ),
                                            )
                                            ...
                                            if (
                                                chan.has_invalid_bit(info)
                                                and not info["DG"][dataGroup]["unique_channel_in_DG"]
                                            ):
                                                # has invalid bit
                                        ...
                                        if not info["DG"][dataGroup]["unique_channel_in_DG"]:
                                            invalid_data = frombuffer(invalid_data.tobytes(), dtype="u1").reshape(
                                                len(invalid_data), invalid_data.dtype.itemsize
                                            )
                                            self.add_channel(
                                                chan.name,
                                                invalid_data,
    ...
    def _get_channel_data4(self, channel_name, raw_data=False):
        ...
        if channel_name in self:
            ...
            if not raw_data:
                return self._convert_channel_data4(self.get_channel(channel_name), channel_name, self.convertTables)[
                    channel_name
                ]
            else:
    ...
    def _convert_channel_data4(channel, channel_name, convert_tables, multi_processed=False, q=None):
        ...
        if conversionField in channel and channel[conversionField]["type"]:  # there is conversion property
            ...
            if conversion_type == 1 and not text_type:
                ...
                vector = _value_to_text_conversion(
                    vector, conversion_parameter["cc_val"], conversion_parameter["cc_ref"]
                )
            elif conversion_type == 8 and not text_type and convert_tables:
                vector = _value_range_to_text_conversion(
                    vector, conversion_parameter["cc_val"], conversion_parameter["cc_ref"]
                )
            elif conversion_type == 9 and text_type and convert_tables:
                vector = _text_to_value_conversion(
                    vector, conversion_parameter["cc_val"], conversion_parameter["cc_ref"]
                )
                ...
                vector = _bitfield_text_table_conversion(
                    vector, conversion_parameter["cc_val"], conversion_parameter["cc_ref"]
                )
        L = dict()
                    ...
                    for channel in self.masterChannelList[masterChannel]:
                        if channel.find("invalid_bytes") == -1:  # not invalid bytes channel
                            ...
                            if channel == masterChannel:
                                ...
                                dg, pointer = self._write4_column(
                                    fid, pointer, cg_cycle_count, masterChannel, True, compression
                                )
                            else:
                                if cg_master_pointer is not None:
                                    ...
                                    dg, pointer = self._write4_column(
                                        fid, pointer, cg_cycle_count, channel, False, compression
                                    )
    ...
    def _write4_column(
        self, fid, pointer, cg_cycle_count, channel, master_channel_flag, compression=False, cg_master_pointer=None
    ):
        ...
        if data is not None and len(data) > 0:
            ...
            if compression or invalid_channel:
                ...
                data_blocks.load(
                    ...
                    cg_cycle_count,
                    pointer,
                    invalid_bytes=blocks["CG"]["cg_inval_bytes"],
                    column_oriented_flag=True,
                )
                dg_start_position = fid.tell()
                dg["DG"] = 0
            else:
            ...
            if not invalid_channel and not compression:
                ...
                pointer = data_blocks.write(fid, data.tobytes(), invalid_data=inval_data, compression_flag=compression)
    ...
    for ref in range(len(cc_ref)):
        if isinstance(cc_ref[ref], CCBlock):
            if cc_ref[ref]["cc_type"] == 3:
                ...
                cc_ref[ref] = lambdify(X, cc_ref[ref]["cc_ref"]["Comment"], modules="numpy", dummify=False)
            elif cc_ref[ref]["cc_type"] == 1:  # linear conversion
                cc_ref[ref] = lambdify(
                    X,
                    "{0}* X + {1}".format(cc_ref[ref]["cc_val"][1], cc_ref[ref]["cc_val"][0]),
                    modules="numpy",
                    dummify=False,
                )
            else:
    ...
    for ref in range(len(cc_ref)):
        if isinstance(cc_ref[ref], CCBlock):
            if cc_ref[ref]["cc_type"] == 3:
                ...
                cc_ref[ref] = lambdify(X, "{0} * X + {1}".format(cc_val[1], cc_val[0]), modules="numpy", dummify=False)
            else:  # identity, no conversion
                cc_ref[ref] = lambdify(X, "1 * X", modules="numpy", dummify=False)
            # Otherwise a string
    # look up in range keys
    temp = []
    for value in vector:
        ...
        for i in range(ref_count):
            ...
            if cc_ref[i]:  # not NIL link
                if cc_ref[i]["cc_type"] == 7:
                    assembled_string += _value_to_text_conversion(bitmask, cc_ref[i]["cc_val"], cc_ref[i]["cc_ref"])
                if cc_ref[i]["cc_type"] == 8:
                    assembled_string += _value_range_to_text_conversion(
                        bitmask, cc_ref[i]["cc_val"], cc_ref[i]["cc_ref"]
                    )
