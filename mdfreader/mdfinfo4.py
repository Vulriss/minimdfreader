...
from .mdf import _open_mdf, dataField, descriptionField, unitField, masterField, masterTypeField, idField, _convert_name
...
si_bus_type = {0: "NONE", 1: "OTHER", 2: "CAN", 3: "LIN", 4: "MOST", 5: "FLEXRAY", 6: "K_LINE", 7: "ETHERNET", 8: "USB"}
# EV cause
...
class IDBlock(dict):
    ...
    def read(self, fid):
        ...
        (
            ...
            id_reserved2,
            self["id_unfi_flags"],
            self["id_custom_unfi_flags"],
        ) = unpack("<8s8s8sIH30s2H", fid.read(64))
        ...
        if self["id_ver"] > 410 and b"UnFin" in self["id_file"]:
            ...
            if self["id_unfi_flags"] & (1 << 4):
                warn("Update of last DL block in each chained list of " "DL blocks required")
            if self["id_unfi_flags"] & (1 << 5):
                warn("Update of cg_data_bytes and cg_inval_bytes in VLSD " "CG block required")
            if self["id_unfi_flags"] & (1 << 6):
                warn("Update of offset values for VLSD channel required " "in case a VLSD CG block is used")

    def write(self, fid):
        """Writes IDBlock"""
        # MDF versionTxt tool reserved version_int
        head = (b"MDF     ", self["id_vers"], b"MDFreadr", b"\0" * 4, self["id_ver"], b"\0" * 30, 0, 0)
...
class HDBlock(dict):
    ...
    def read(self, fid=None):
        ...
        (
            ...
            self["hd_start_distance"],
        ) = unpack("<4sI9Q2h4B2d", fid.read(104))
...
class FHBlock(dict):
    ...
    def write(self, fid):
        ...
        # time flags, reserved)
        data_bytes = (b"##FH", 0, 56, 2, 0, self["MD"], int(time() * 1e9), 0, 0, 1, b"\0" * 3)
...
class CHBlock(dict):
    ...
    def __init__(self, fid, pointer):
        ...
        (self["ch_ch_next"], self["ch_ch_first"], self["ch_tx_name"], self["ch_md_comment"]) = unpack(
            "<4Q", fid.read(32)
        )
        n_links = self["link_count"] - 4
        self["ch_element"] = unpack("<{}Q".format(n_links), fid.read(n_links * 8))
        (self["ch_element_count"], self["ch_type"], ch_reserved) = unpack("<IB3s", fid.read(8))
...
class CommentBlock(dict):
    ...
    def read_tx(self, fid, pointer):
        ...
        if pointer > 0:
            fid.seek(pointer)
            # block header
            (self["id"], reserved, self["length"], self["link_count"]) = _HeaderStruct.unpack(fid.read(24))
...
class DGBlock(dict):
    ...
    def write(self, fid):
        ...
        data_bytes = (b"##DG", 0, 64, 4, self["DG"], self["CG"], self["data"], 0, 0, b"\0" * 7)
...
class CGBlock(dict):
    ...
    def read_cg(self, fid, pointer):
        ...
        (
            ...
            self["cg_reserved"],
            self["cg_data_bytes"],
            self["cg_invalid_bytes"],
        ) = _CGStruct2.unpack(fid.read(32))
    ...
    def read_cn(self, **kargs):
        if kargs["pointer"] != 0 and kargs["pointer"] is not None:
            ...
            (
                ...
                self["cn_md_unit"],
                self["cn_md_comment"],
            ) = _CNStruct1.unpack(kargs["fid"].read(88))
            ...
            (
                ...
                cn_limit_ext_min,
                cn_limit_ext_max,
            ) = _CNStruct2.unpack(kargs["fid"].read(72))
            if self["cn_attachment_count"] > 0:
                ...
                if self["cn_attachment_count"] > 1:
                    ...
                    for at in range(self["cn_attachment_count"]):
                        self["attachment"][at] = ATBlock(kargs["fid"], self["cn_at_reference"][at])
                else:
                    self["cn_at_reference"] = links
                    self["attachment"][0] = ATBlock(kargs["fid"], self["cn_at_reference"])
    ...
    def read_cc(self, fid, pointer):
        ...
        if pointer != 0 and pointer is not None:
            ...
            (
                ...
                self["cc_phy_range_max"],
            ) = _CCStruct2.unpack(fid.read(24))
...
class CABlock(dict):
    ...
    def read(self, fid, pointer):
        ...
        if pointer != 0 and pointer is not None:
            ...
            (
                ...
                self["ca_invalid_bit_pos_base"],
            ) = unpack("2BHIiI", fid.read(16))
            ...
            if 1 << 5 & self["ca_flags"]:  # bit5
                self["ca_axis_value"] = _mdf_block_read(fid, _REAL, self["SNd"])
            if self["ca_storage"] >= 1:
                self["ca_cycle_count"] = _mdf_block_read(fid, _UINT64, self["PNd"])
            ...
            if 1 << 0 & self["ca_flags"]:  # bit 0
                self["ca_dynamic_size"] = _mdf_block_read(fid, _LINK, self["ca_ndim"] * 3)
            if 1 << 1 & self["ca_flags"]:  # bit 1
                self["ca_input_quantity"] = _mdf_block_read(fid, _LINK, self["ca_ndim"] * 3)
            if 1 << 2 & self["ca_flags"]:  # bit 2
                self["ca_output_quantity"] = _mdf_block_read(fid, _LINK, 3)
            if 1 << 3 & self["ca_flags"]:  # bit 3
                self["ca_comparison_quantity"] = _mdf_block_read(fid, _LINK, 3)
            if 1 << 4 & self["ca_flags"]:  # bit 4
                self["ca_cc_axis_conversion"] = _mdf_block_read(fid, _LINK, self["ca_ndim"])
    ...
    def write(self, fid):
        # default CN template
        data_bytes = (b"##CA", 0, 48 + 8 * self["ndim"], 1, 0, 0, 0, self["ndim"], 0, self["byte_offset_base"], 0)
...
class ATBlock(dict):
    ...
    def __init__(self, fid, pointer):
        ...
        if pointer != 0 and pointer is not None:
            ...
            ) = unpack("<4sI6Q2HI16s2Q", fid.read(96))
            ...
            if self["ev_attachment_count"] > 0:
                if self["ev_attachment_count"] > 1:
                    self["ev_at_reference"] = links[
                        self["ev_scope_count"] : self["ev_scope_count"] + self["ev_attachment_count"]
                    ]
                else:
                    self["ev_at_reference"] = links
            # post treatment
...
class LDBlock(dict):
    ...
    def read_ld(self, fid, link_count):
        ...
        self["list_data"] = {}
        self["list_data"][0] = list(unpack("<{}Q".format(link_count - 1), fid.read(8 * (link_count - 1))))
        (self["flags"], self["count"]) = unpack("<2I", fid.read(8))
        if self["flags"]:  # flags existing
            if self["flags"] & (1 << 31):  # invalid data present
                self["inval_data"] = {}
                self["inval_data"][0] = [
                    self["list_data"][0].pop(i) for i in range((link_count - 1) // 2, link_count - 1)
                ]
            if self["flags"] & 0b1:  # equal length data list
                ...
                self["sample_offset"] = unpack("<{}Q".format(self["count"]), fid.read(8 * self["count"]))
            if self["flags"] & 0b10:  # time values
                self["time_values"] = unpack("<{}Q".format(self["count"]), fid.read(8 * self["count"]))
            if self["flags"] & 0b100:  # angle values
                self["angle_values"] = unpack("<{}Q".format(self["count"]), fid.read(8 * self["count"]))
            if self["flags"] & 0b1000:  # distance values
                self["distance_values"] = unpack("<{}Q".format(self["count"]), fid.read(8 * self["count"]))
    ...
    def write(self, fid, data, invalid_data=None, compression_flag=False):
        ...
        if compression_flag:
            ...
            for counter, (n_record_chunk, chunk_size) in enumerate(self["chunks"]):
                position, dl_data = self.write_DZ(
                    fid,
                    pointer,
                    data,
                    dl_data,
                ...
                if position is not None:
                    ...
                    pointer, dl_data = self.write_DIV(
                        fid,
                        pointer,
                        DVBlock(),
                        data,
                ...
                if invalid_data is not None:
                    position, dl_invalid_data = self.write_DZ(
                        fid,
                        pointer,
                        invalid_data,
                        dl_invalid_data,
                        counter,
                        data_invalid_pointer,
                        self["invalid_bytes"],
                        n_record_chunk,
                        dz_zip_type,
                        b"DI",
                    ...
                    if position is not None:
                        ...
                        pointer, dl_invalid_data = self.write_DIV(
                            fid,
                            pointer,
                            DIBlock(),
                            invalid_data,
    ...
    def write_DIV(
        self, fid, pointer, block, data, dl_data, counter, data_pointer, record_length, chunk_size, n_records
    ):
...
class DLBlock(dict):
    ...
    def read_dl(self, fid, link_count):
        ...
        self["next"] = unpack("<Q", fid.read(8))[0]
        self["list_data"] = {}
        self["list_data"][0] = unpack("<{}Q".format(link_count - 1), fid.read(8 * (link_count - 1)))
        (self["flags"], dl_reserved, self["count"]) = unpack("<B3sI", fid.read(8))
        if self["flags"] & 0b1:  # equal length data list
            self["equal_length"] = unpack("<Q", fid.read(8))[0]
        else:  # data list defined by byte offset
            self["offset"] = unpack("<{}Q".format(self["count"]), fid.read(8 * self["count"]))
        if self["flags"] & 0b10:  # time values
            self["time_values"] = unpack("<{}Q".format(self["count"]), fid.read(8 * self["count"]))
        if self["flags"] & 0b100:  # angle values
            self["angle_values"] = unpack("<{}Q".format(self["count"]), fid.read(8 * self["count"]))
...
class HLBlock(dict):
    ...
    def read_hl(self, fid):
        (self["hl_dl_first"], self["hl_flags"], self["hl_zip_type"], hl_reserved) = _HLStruct.unpack(fid.read(16))
...
class Info4(dict):
    ...
    def read_cg_blocks(self, fid, dg, channel_name_list=False, minimal=0):
        ...
        if self["DG"][dg]["dg_cg_first"]:
            ...
            vlsd_cg_block = self.read_cg_block(
                fid, dg, cg, self["DG"][dg]["dg_cg_first"], vlsd_cg_block, channel_name_list=False, minimal=0
            )

            if (
                self["CN"][dg][cg]
                and self["CG"][dg][cg]["unique_channel_in_CG"]
                and not self["CG"][dg][cg]["cg_cg_next"]
            ):
            ...
            while self["CG"][dg][cg]["cg_cg_next"]:
                ...
                self["CC"][dg][cg] = dict()
                vlsd_cg_block = self.read_cg_block(
                    fid, dg, cg, self["CG"][dg][cg - 1]["cg_cg_next"], vlsd_cg_block, channel_name_list=False, minimal=0
                )
    ...
    def read_cn_blocks(self, fid, dg, cg, channel_name_list=False, minimal=0):
        ...
        vlsd = False
        mlsd_channels = []
        cn, mlsd_channels, vlsd = self.read_cn_block(
            fid, self["CG"][dg][cg]["cg_cn_first"], dg, cg, mlsd_channels, vlsd, minimal, channel_name_list
        )
        ...
        while self["CN"][dg][cg][cn]["cn_cn_next"]:
            cn, mlsd_channels, vlsd = self.read_cn_block(
                fid, self["CN"][dg][cg][cn]["cn_cn_next"], dg, cg, mlsd_channels, vlsd, minimal, channel_name_list
            )
    ...
    def _unique_channel_name(self, fid, name, dg, cg, cn):
        ...
        if name in self["ChannelNamesByDG"][dg]:  # for unsorted data
            if self["CN"][dg][cg][cn]["cn_si_source"]:
                ...
                source_name = cn
            name = "{0}_{1}_{2}_{3}".format(name, dg, cg, source_name)
            ...
            if self["CN"][dg][cg][cn]["cn_si_source"]:
                ...
                source_name = dg
            name = "{0}_{1}_{2}".format(name, dg, source_name)
