        ...
        for cg in range(self["DGBlock"][dg]["numberOfChannelGroups"]):
            ...
            for channel in range(self["CGBlock"][dg][cg]["numberOfChannels"]):
                ...
                if self["CNBlock"][dg][cg][channel]["pointerToASAMNameBlock"] > 0:
                    # long name of signal
                    long_signal_name = read_tx_block(fid, self["CNBlock"][dg][cg][channel]["pointerToASAMNameBlock"])
                ...
                if signal_name in self["ChannelNamesByDG"][dg]:  # for unsorted data
                    ...
                    if pointer:
                        ...
                        temp = dict()
                        temp["tail"] = channel
                    self["CNBlock"][dg][cg][channel]["signalName"] = "{0}_{1}_{2}_{3}".format(
                        signal_name, dg, cg, temp["tail"]
                    )
                    ...
                    if "{0}_{1}".format(signal_name, temp["tail"]) not in self["allChannelList"]:
                        ...
                        self["CNBlock"][dg][cg][channel]["signalName"] = "{0}_{1}_{2}".format(
                            signal_name, dg, temp["tail"]
                        )
                else:
                ...
                self["CNBlock"][dg][cg][channel]["ChannelCommentBlock"] = read_tx_block(
                    fid, self["CNBlock"][dg][cg][channel]["pointerToChannelCommentBlock"]
                )

                # Read data Text block info into structure
                self["CNBlock"][dg][cg][channel]["SignalIdentifierBlock"] = read_tx_block(
                    fid, self["CNBlock"][dg][cg][channel]["pointerToSignalIdentifierBlock"]
                )
                ...
                if self["CNBlock"][dg][cg][channel]["pointerToConversionFormula"] == 0:
                    ...
                    self["CCBlock"][dg][cg][channel] = read_cc_block(
                        fid, self["CNBlock"][dg][cg][channel]["pointerToConversionFormula"]
                    )
                ...
                for channel in range(self["CGBlock"][dataGroup][channelGroup]["numberOfChannels"]):
                    ...
                    if signal_name in names_set:
                        ...
                        if pointer:
                            ...
                            temp = dict()
                            temp["tail"] = channel
                        self["CNBlock"][dataGroup][channelGroup][channel]["signalName"] = "{0}_{1}".format(
                            signal_name, temp["tail"]
                        )
                        warn(
...
def read_hd_block(fid, pointer, version=0):
    ...
    if pointer != 0 and pointer is not None:
        ...
        if version < 320:
            ...
            ) = unpack("<2sH3IH10s8s32s32s32s32sQhH32s", fid.read(208))
...
def read_cc_block(fid, pointer):
    ...
    if pointer != 0 and pointer is not None:
        ...
        if temp["cc_type"] == 0:  # Parametric, Linear: Physical = Integer*P2 + P1
            (temp["conversion"]["P1"], temp["conversion"]["P2"]) = unpack("2d", fid.read(16))
            ...
            for pair in range(temp["numberOfValuePairs"]):
                ...
                (temp["conversion"][pair]["int"], temp["conversion"][pair]["phys"]) = unpack("2d", fid.read(16))
            ...
            for pair in range(temp["numberOfValuePairs"]):
                temp["conversion"][pair] = dict()
                (temp["conversion"][pair]["int"], temp["conversion"][pair]["text"]) = unpack("d32s", fid.read(40))
...
def read_ce_block(fid, pointer):
    ...
    if pointer != 0 and pointer is not None:
        ...
        if temp["extension_type"] == 2:
            (temp["n_module"], temp["address"], temp["description"], temp["ECU_id"]) = unpack("HI80s32s", fid.read(120))
            ...
            (temp["CAN_id"], temp["CAN_channel_id"], temp["message"], temp["sender"]) = unpack("2I36s36s", fid.read(80))
            ...
            for cn in info["CNBlock"][dg][cg]:
                ...
                if channel_list is None or name in channel_list:
                    ...
                    if "signalDescription" in info["CNBlock"][dg][cg][cn]:
                        mdf_dict[name][descriptionField] = info["CNBlock"][dg][cg][cn]["signalDescription"]
                    else:
